...
class Database:
    ...
    async def add_user(self, full_name, username, telegram_id, phone_number, latitude, longitude, create_dt, k1):
        ...
        return await self.execute(sql, full_name, username, telegram_id, phone_number, latitude, longitude, create_dt,
                                  k1,
                                  fetchrow=True)

    async def update_user_bool(self, user_id, bool, ):
    ...
    async def add_order(self, count, create_dt, product_id, user_id, k):
        ...
        return await self.execute(sql, count, create_dt, product_id, user_id, k,
                                  fetchrow=True)
    ...
    async def delete_user(self, telegram_id):
        await self.execute("DELETE FROM products_user WHERE telegram_id=$1", telegram_id, execute=True)

    async def delete_order(self, user_id):
        await self.execute("DELETE FROM products_order WHERE user_id=$1", user_id, execute=True)

    async def delete_order1(self, oder_id):
        await self.execute("DELETE FROM products_order WHERE id=$1", oder_id, execute=True)
